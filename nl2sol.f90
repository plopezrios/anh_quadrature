...
!          that 1 + machep .gt. 1 .and. 1 - machep < 1.             !
...
!-------------------------------------------------------------------!
 ...
 REAL(dp) fn_val
 select case (k)
 case(1) ; fn_val=tiny(1._dp)
 case(2) ; fn_val=sqrt(tiny(1._dp))
 case(3) ; fn_val=epsilon(1._dp)
 case(4) ; fn_val=sqrt(epsilon(1._dp))
 case(5) ; fn_val=sqrt(huge(1._dp))
 case(6) ; fn_val=huge(1._dp)
 case default ; fn_val=0.d0
  write(6,*)'Illegal argument to function RMDCON.'
  stop
  !call errstop('RMDCON','Illegal argument to function RMDCON in NL2SOL &
  ! &module.')
 end select
 END FUNCTION rmdcon
...
INTEGER,PARAMETER :: afctol=31,decfac=22,dstnrm=2,dst0=3,dstsav=18,           &
 &f=10,fdif=11,flstgd=12,f0=13,gtslst=14,gtstep=4,incfac=23,lmax0=35,nreduc=6, &
 &plstgd=15,preduc=7,radfac=16,rdfcmn=24,rdfcmx=25,reldx=17,rfctol=32,stppar=5,&
 &tuner1=26,tuner2=27,tuner3=28,xctol=33,xftol=34
